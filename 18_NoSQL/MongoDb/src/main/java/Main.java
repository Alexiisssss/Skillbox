...
public class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    System.out.println("Kоличество студентов старше 40 лет: " + collection.countDocuments(
        BsonDocument.parse("{age: {$gt: 40}}")));
...
//            System.out.println("Имя самого молодого студента: "  + document.getString("name"));
    ...
    collection.find().sort(query).forEach((Consumer<Document>) document -> {
      if (document.getInteger("age").equals(age)) {
...
//            System.out.println("Cписок курсов самого старого студента: "  + document.get("CourseList"));
    ...
    collection.find().sort(query).forEach((Consumer<Document>) document -> {
      if (document.getInteger("age").equals(age)) {
        System.out.println(
            document.getString("name") + ", возраст: " + document.getInteger("age") + ", курсы: "
                + document.get("CourseList"));
      }
    });
  }
